...
from abc import ABC
from typing import Generator, Set, Type, cast, Optional, List, Tuple, Dict, Any
import json
...
from packages.dvilela.protocols.kv_store.dialogues import KvStoreDialogue, KvStoreDialogues
...
class PrepareTweetsBehaviour(TsunamiBaseBehaviour):
    ...
    def build_tweets(self) -> Generator[None, None, List[str]]:
        ...
        from_block = int(db_data.get("from_block") or self.params.initial_block_ethereum)
        ...
        for event in events:
            ...
            if response.status_code != HTTP_OK:
                self.context.logger.info(f"Failed to download token data: {response.status_code}")
            ...
            tweet = yield from self.build_tweet(
                user_prompt
            )
    ...
    def get_events(self, contract_id, contract_address, event, from_block) -> Generator[None, None, Tuple[Optional[List], Optional[int]]]:
    ...
    def get_token_uri(self, contract_id, contract_address, unit_id) -> Generator[None, None, Optional[str]]:
        """Get registries events"""
    ...
    def build_tweet(self, user_prompt: str) -> Generator[None, None, Optional[str]]:
        ...
        while attempts < MAX_TWEET_ATTEMPTS:
            ...
            response = yield from self._call_llama(system_prompt=system_prompt, user_prompt=user_prompt)
            ...
            self.context.logger.error(f"Tweet is too long [{tweet_len}]: {tweet_attempt}")
...
class PublishTweetsBehaviour(TsunamiBaseBehaviour):
    ...
    def async_act(self) -> Generator:
        ...
        with self.context.benchmark_tool.measure(self.behaviour_id).local():
            ...
            tweets = [t for t in tweets if t["twitter_published"] and t["farcaster_published"]]
